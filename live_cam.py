    ...
    for rect in rects:
        ...
        if ear < EYE_AR_THRESH:
            ...
            if COUNTER >= EYE_AR_CONSEC_FRAMES:
                ...
                if num<3:
                    time=datetime.now()
                    cv2.imwrite("drowsyfaces/newimage{}.jpg".format(time),frame)
        if ear>EYE_AR_THRESH:
            COUNTER=0
            num=0
        ...
        if mar>= MOUTH_AR_THRESH or distance>LIP_DIST_THRESH:
            COUNTER1 +=1
            #cv2.putText(frame, " MOUTH  ALERT!", (10, 100),cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)
            if COUNTER1 >= 11:
                print("yes")
                #yawning +=1
                cv2.putText(frame, " MOUTH  ALERT!", (10, 60),cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)
